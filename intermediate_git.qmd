...
the same commit; we want a clean history.
...
::: aside
Note: `--amend` replaces the
old commit with a new one (complete with new hash), effectively **rewriting
...
Return to `pressure-eqn` and add a docstring and input validation to your
...
You still have one commit on `pressure-eqn`, but now it contains the full
implementation and has a different hash.
...
Resolve the conflict cleanly in your editor, `add` the result, and `continue` the rebase
until complete.
...
change "hunk" and asking what we should do:
...
These are separate hunks in the same file. Use `git add -p` to stage and
commit only the second change, then stage and commit the first:
...
it was passing before. Manual search would take forever.
