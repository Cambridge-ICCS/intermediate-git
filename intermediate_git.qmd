...
::: {.fragment}
:::{.callout-warning}
`--amend` replaces the old **previous** commit (complete with new hash), effectively **rewriting
...
:::{.callout-tip}
Since we're only modifying a tracked file we can stage all changes with:
```txt
git add -u
```
:::
...
:::{.callout-important}
Do **NOT** commit this work yet - we are about to be interrupted.
:::
...
:::{.callout-tip}
We could use `pytest test/unit/test_moisture.py` to only run the failing tests.
:::
